...
<body>
    ...
    <div class="container">
        <main>
            <div class="card">
                <h2><i class="fas fa-handshake"></i> Dashboard de Vendas</h2>
                <div class="metrics-grid">
                    <div class="metric-card"><div class="label">Total de Vendas</div><div class="value" id="totalVendas">0</div></div>
                    <div class="metric-card"><div class="label">Taxa de Conversão</div><div class="value" id="conversionRate">0%</div></div>
                    <div class="metric-card"><div class="label">Valor Total (R$)</div><div class="value" id="totalValue">R$ 0</div></div>
                    <div class="metric-card"><div class="label">Leads Processados</div><div class="value" id="leadsProcessed">0</div></div>
                </div>
            </div>

            <div class="card">
                <h2><i class="fas fa-chart-bar"></i> Vendas por Campanha</h2>
    ...
    <script>
        ...
        async function loadVendasDashboard() {
            try {
                ...
                const response = await fetchWithAuth(`${API_BASE_URL}/dashboard/vendas`);
        ...
        function updateMetrics(data) {
            ...
            document.getElementById('totalValue').textContent = 'R$ ' + (stats.total_value || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 });
            document.getElementById('leadsProcessed').textContent = stats.leads_processed || 0;
        }
        ...
        function renderVendas(data) {
            ...
            tbody.innerHTML = data.campanhas.map(c => `
                <tr>
                    ...
                    <td>R$ ${(c.valor_vendas || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td>
                </tr>
            `).join('');
        }
